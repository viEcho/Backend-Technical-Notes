...
--
    ...
    LinkedList  ArrayList   HashSet  LinkedHashSet  HashMap  Hashtable LinkedHashMap就可以了。       
...
###好了，话不多说，看看这些集合的不同吧：     
    ...
    其中HashMap允许key值为null,HashTable既不允许key为null值也不允许value值为null.其中HashTable是线程安全的        
    （它的每个方法中都加入了Synchronize方法），HashMap不是线程安全的，所以效率上HashMap的比HashTable更高     
...
（4）多功能实用工具：Redis是一个多实用的工具，可以在多个用例如缓存，消息，队列使用(Redis原生支持发布/订阅)，       
